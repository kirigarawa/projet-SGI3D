...
<style>
  ...
  .site-node{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:var(--radius);overflow:hidden;transition:all .3s}
  ...
  .node-link{display:flex;align-items:center;gap:.6rem;color:rgba(255,255,255,.75);text-decoration:none;font-size:.88rem;padding:.4rem .6rem;border-radius:8px;transition:all .2s}
...
<nav class="site-nav">
  ...
  <div class="nav-links">
    ...
    <?php if ($isLoggedIn): ?>
      <a href="dashboard.php" class="btn-nav">👤 <span><?= htmlspecialchars(explode(' ', $username)[0]) ?></span></a>
    <?php else: ?>
...
<div class="sitemap-wrap">
  ...
  <div class="sitemap-tree">
    ...
    <div class="site-node">
      ...
      <div class="node-links">
        ...
        <a href="#"           class="node-link"><span class="lk-icon">🎥</span> Flux caméras en direct</a>
    ...
    <div class="site-node">
      <div class="node-header">
        <div class="node-icon">⚙️</div>
        <div><h2>API & Export</h2><p>Backend et portabilité</p></div>
      </div>
      <div class="node-links">
        <a href="#"            class="node-link"><span class="lk-icon">⚙️</span> api.php <span class="badge-admin">Backend</span></a>
        <a href="dashboard.php" class="node-link"><span class="lk-icon">📤</span> Export CSV via Dashboard</a>
        ...
        <a href="logout.php"   class="node-link"><span class="lk-icon">🚪</span> logout.php</a>
        <a href="sitemap.php"  class="node-link"><span class="lk-icon">🗺️</span> sitemap.php (cette page)</a>
      </div>
    </div>
  </div>
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            <?php
            $pages = [
              ['index.php',    '🏠 Accueil',       'public', 'Page principale avec monitoring global'],
              ['login.php',    '🔐 Connexion',      'public', 'Authentification et création de compte'],
              ['printers.php', '🖨️ Imprimantes',   'login',  'Monitoring OctoPrint + contrôle en temps réel'],
              ...
              ['style.css',    '🎨 Styles',         'file',   'Design tokens, sidebar, composants UI'],
            ];
            $badgeMap = [
              ...
              'server' => '<span class="badge badge-neutral">Serveur</span>',
              'file'   => '<span class="badge badge-neutral">Fichier</span>',
            ];
            foreach ($pages as [$file, $label, $access, $desc]):
              $isFile  = in_array($access, ['server', 'file']);
              $isCurrent = ($file === 'sitemap.php');
            ?>
            <tr>
              <td><code><?= htmlspecialchars($file) ?></code></td>
              <td><?= $label ?></td>
              <td><?= $badgeMap[$access] ?? '' ?></td>
              <td><?= htmlspecialchars($desc) ?></td>
              <td>
                <?php if ($isFile): ?>
                  —
                <?php elseif ($isCurrent): ?>
                  <a href="sitemap.php" class="btn btn-ghost btn-xs">Ici</a>
                <?php else: ?>
                  <a href="<?= htmlspecialchars($file) ?>" class="btn btn-ghost btn-xs">Ouvrir</a>
                <?php endif; ?>
              </td>
            </tr>
            <?php endforeach; ?>
  ...
  <div class="legend">
    ...
    <span>🔵 Login = nécessite une session PHP active</span>
    <span>⚙️ Serveur = fichier backend, non accessible directement</span>
  </div>

  <!-- Instructions déploiement -->
  <div class="card-dark" style="padding:2rem;margin-top:2rem">
    <h2 style="margin-bottom:1rem">🚀 Comment déployer SGI3D</h2>
    <ol style="line-height:2;color:rgba(255,255,255,.8);padding-left:1.5rem">
      <li>Copiez tous les fichiers <code>.php</code>, <code>style.css</code> et <code>api.php</code> dans votre répertoire web (ex. <code>/var/www/html/sgi3d/</code>)</li>
      <li>Importez le schéma SQL dans votre base MySQL et configurez les accès dans <code>config.php</code></li>
      <li>Assurez-vous que PHP a accès au réseau local pour joindre les instances OctoPrint</li>
      <li>Ouvrez <code>index.php</code> dans votre navigateur et connectez-vous avec votre compte admin</li>
      <li>Toutes les données sont stockées en base MySQL — plus aucun <code>localStorage</code> ✅</li>
    </ol>
